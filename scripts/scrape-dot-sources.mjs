...
const PRIORITY=path.join(DATA,'priority-current.json');
const PRIORITY_JS=path.join(DATA,'priority-current.js');
const DOT_JSON=path.join(DATA,'dot-current.json');
...
const LABEL={VDOT:'VDOT CABB',NCDOT:'NCDOT'};
const CENT={VA:[37.6,-78.6],NC:[35.5,-79.4]};
const clean=s=>String(s||'').replace(/\u00a0/g,' ').replace(/\s+/g,' ').trim();
const norm=s=>clean(s).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
const dayKey=s=>String(s||'').slice(0,10);
const recKey=o=>`${o.source}|${norm(o.s)}|${norm(o.n)}|${dayKey(o.d)}`;
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
function readJson(file,fallback){return fs.readFile(file,'utf8').then(JSON.parse).catch(()=>fallback)}
function parseUsDate(text){const m=clean(text).match(/\b(\d{1,2})[/-](\d{1,2})[/-](20\d{2})\b/);if(!m)return null;const d=new Date(Date.UTC(+m[3],+m[1]-1,+m[2],17,0,0));return Number.isFinite(d.getTime())?d:null}
function parseNamedDate(text){const d=new Date(clean(text));return Number.isFinite(d.getTime())?d:null}
function futureDate(text){const d=parseUsDate(text)||parseNamedDate(text);return d&&d>NOW?d:null}
...
function letStatus(text){const m=clean(text).match(/Status\s*(Advertised|Anticipated)/i);return m?.[1]||null}
function letType(text){const t=clean(text);const m=t.match(/Type\s*([A-Za-z0-9 -]+)/i);if(m)return clean(m[1]).replace(/\s+(Status|Let Date).*$/i,'');if(/Central/i.test(t))return'Central';if(/Division\s*\d+/i.test(t))return t.match(/Division\s*\d+/i)[0];if(/Design[- ]?Build/i.test(t))return'Design-Build';if(/Roadside/i.test(t))return'Roadside';return'NCDOT';}
async function ncdotLetCards(page){return page.evaluate(()=>{const els=[...document.querySelectorAll('li,tr,article,section,div')],seen=new Set(),out=[];for(const el of els){const text=(el.innerText||'').replace(/\s+/g,' ').trim();if(!/Status\s*(Advertised|Anticipated)/i.test(text)||!/(\d{1,2}[-/]\d{1,2}[-/]20\d{2}|[A-Z][a-z]{2}\s+\d{1,2},\s+20\d{2})/.test(text))continue;if(text.length>900)continue;const child=[...el.children].some(c=>/Status\s*(Advertised|Anticipated)/i.test((c.innerText||''))&&(c.innerText||'').length<text.length*.9);if(child)continue;const links=[...el.querySelectorAll('a[href]')].map(a=>({text:(a.innerText||'').replace(/\s+/g,' ').trim(),href:a.href}));const key=text.slice(0,300);if(!seen.has(key)){seen.add(key);out.push({text,links})}}return out;});}
