...
async function description(noticeId){const u=new URL(BASE);u.searchParams.set('noticeid',noticeId);u.searchParams.set('api_key',API_KEY);const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);try{const res=await fetch(u,{signal:ctl.signal,headers:{Accept:'application/json,text/plain,*/*','User-Agent':'MC3-Construction-Pursuit-Radar/1.0'}});if(!res.ok)throw new Error(`HTTP ${res.status}`);const body=await res.text();try{return strings(JSON.parse(body)).join(' ')}catch{return body}}finally{clearTimeout(timer)}}
...
for(const row of rows){while(row.length<15)row.push('');const id=row[0],updated=row[7]||row[5]||'';if(row[14]){cache.records[id]={updated,rom:row[14],checkedAt:RUN};continue}const c=cache.records[id];if(c&&c.updated===updated&&!c.error){row[14]=c.rom||'';cached++;continue}work.push({row,id,updated})}
await mapLimit(work,12,async item=>{try{const text=await description(item.id);const rom=romFrom(text);item.row[14]=rom;cache.records[item.id]={updated:item.updated,rom,checkedAt:RUN};fetched++}catch(e){cache.records[item.id]={updated:item.updated,rom:'',checkedAt:RUN,error:String(e.message||e).slice(0,120)};failed++}});
